<template>
    <div class="dropdown-like-grp">
        <div class="drop-btn verde-claro" >
            <label id="drop_canal_mini_label">Loja A</label>
            <i class="bi bi-chevron-down"></i>
        </div>
        <div class="dropdown-body" id="drop_canais_mini">
            ...
            <div class="dropdown-conteudo" id="bico-content-1">
                <section id="chat-area-msgs-section" 
                 class="classe-dropdown" v-for="(rows, idn) in lista_canais" :key="idn"> 
                    <div v-on:click="select_this_value(rows.canal)" v-bind='title=rows.descrip'> {{ rows.canal }} </div>
                </section>
            </div>
        </div>
    </div>
  ...
  data: function () {
    ...
    methods:{
        SH_show_hide_me: function(){
            ...
            var dropdow=document.getElementById('drop_canais_mini');
        ...
        select_this_value: (isso) =>{
            document.getElementById('drop_canal_mini_label').innerHTML=isso;
