<template>
    <div class="dropdown-like-grp">
        <div class="drop-btn roxo-claro"  @click="SH_show_hide_me('drop_segmentacao_mini')" >
            <label id="drop_segmentacao_mini_label">Segmentações</label>
            <i class="bi bi-chevron-down"></i>
        </div>
        <div class="dropdown-body" id="drop_segmentacao_mini">
            ...
            <div class="dropdown-conteudo" id="bico-content-1">
                ...
                <section id="chat-area-msgs-section" 
                 class="classe-dropdown" v-for="(rows, idn) in lista_segmentacoes" :key="idn"> 
                    <p><input class="segm-checkbox" type="checkbox" v-bind:id="'segm_v_'+rows.cod_seg" v-on:click="set_this_segment(rows.cod_seg)"> {{rows.descrip_seg }} </p>
                    <hr>
                </section>
            </div>
        </div>
  ...
  data: function () {
    ...
    methods:{
        SH_show_hide_me: function(){
            ...
            var dropdow=document.getElementById('drop_segmentacao_mini');
